...
public final class Board {
    ...
    public String toString() {
        ...
        builder.append("\n"); // Add a newline to start
        for (int i = 0; i < BoardUtils.NUM_TILES; i++) {
            // At the start of a new row, print the rank number (8, 7, etc.)
            if (i % 8 == 0) {
                builder.append(" " + (8 - (i / 8)) + " |");
            }

            final String tileText = prettyPrint(this.boardPieces[i]);
            builder.append(String.format("%3s", tileText));

            // At the end of a row, print the rank number again and a newline
            if ((i + 1) % 8 == 0) {
                builder.append(" | " + (8 - (i / 8)) + "\n");
            }
        }
        // Add the file letters (a, b, c...) at the bottom
        builder.append("   -------------------------\n");
